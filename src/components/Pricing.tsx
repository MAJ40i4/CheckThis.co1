    ...
    <section id="pricing" className="py-16 md:py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-10 md:mb-12">
          ...
          <p className="text-sm md:text-base text-slate-600 mb-8 max-w-2xl mx-auto">{t.subtitle}</p>
          ...
          <div className="inline-flex bg-slate-100 p-1 rounded-xl">
             {(['PLN', 'EUR', 'USD'] as Currency[]).map((c) => (
               <button
                 ...
                 className={`px-3 md:px-4 py-2 rounded-lg text-xs font-bold transition-all ${currency === c ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
               >
                 {c}
               </button>
             ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((tier, i) => {
            ...
            return (
              <div key={i} className={`p-6 rounded-[2rem] flex flex-col border transition-all duration-300 relative ${tier.highlight ? 'border-emerald-500 bg-emerald-50/50 shadow-xl md:scale-105 z-10' : 'border-slate-200 bg-white hover:border-emerald-200'}`}>
                {tier.highlight && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-emerald-500 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Most Popular</div>}
                ...
                <p className="text-[10px] text-slate-500 mb-6 font-medium uppercase tracking-wider">{tier.desc}</p>
